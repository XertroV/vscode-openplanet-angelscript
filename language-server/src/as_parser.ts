...
export function ParseModuleAndDependencies(module : ASModule, visited : Set<ASModule> = null, debug: boolean = false)
{
    if (_DEBUG)
        debug = true;
    ...
    if (!module.loaded)
        return;
    if (!module.parsed)
        ParseModule(module, debug);

    visitList.add(module);
    for (let importedModule of module.importedModules)
        ParseModuleAndDependencies(importedModule, visitList, debug);
}
...
function GenerateTypeInformation(scope : ASScope, _previous?: ASElement)
{
    ...
    if (_previous && _previous instanceof ASStatement && _previous.ast)
    {
        ...
        else if (_previous.ast.type == node_types.SetStatement) {
            // either: statement before set was var decl, or get block, then get statement, then var decl
            let varDecl = ((scope.parentscope.previous as ASStatement)?.ast?.type == node_types.VariableDecl) ? scope.parentscope.previous : scope.parentscope.previous;
            if (!(varDecl instanceof ASStatement) || varDecl.ast.type != node_types.VariableDecl) {
                console.warn("_previous.ast.type == node_types.SetStatement: Expected scope.prev or scope.prev.prev to be a variable declaration!")
                console.warn("varDecl.ast.type: " + (varDecl as ASStatement)?.ast?.type)
            } else {
                let valueVar = new ASVariable;
                valueVar.name = "value";
                valueVar.typename = GetQualifiedTypename(varDecl.ast.typename);
                valueVar.node_typename = varDecl.ast.typename;
                valueVar.start_offset_type = varDecl.end_offset - 5;
                valueVar.start_offset_name = varDecl.end_offset - 5;
                valueVar.start_offset_expression = varDecl.end_offset - 5;
                valueVar.end_offset_type = varDecl.end_offset;
                valueVar.end_offset_name = varDecl.end_offset;
                valueVar.end_offset_expression = varDecl.end_offset;
                scope.variables.push(valueVar);
                scope.variablesByName.set(valueVar.name, valueVar);
            }
        }
    ...
    for (let i = 0; i < scope.statements.length; ++i)
    {
        ...
        const ast_type = statement.ast.type;
        switch (ast_type)
        {
            ...
            case node_types.DefaultCaseStatement:
            {
                if (!statement.generatedTypes && statement.ast.children[statement.ast.children.length-1])
                {
                    ...
                    let subscope = MoveStatementToSubScope(scope, statement, statement.ast.children[statement.ast.children.length-1], false);
                }
            }
            break;
            ...
            case node_types.DelegateDecl:
            {
                ...
                if (signature)
                {
                    ...
                    dbtype.isStruct = true;
                    if (ast_type == node_types.EventDecl)
                        dbtype.isEvent = true;
                    else
    ...
    switch (node.type)
    {
        ...
        */
        ...
        case node_types.FuncdefDefinition: {
            ...
            // other stuff goes around line 2100; see: scope.previous.ast.type == node_types.FunctionDecl

            // if (signature?.name?.value) {
            //     let funcdefMethod = AddDBMethod(scope, signature.name.value);
            //     let arg = new typedb.DBArg();
            //     arg.fromJSON({name: "func", typename: "Function"});
            //     funcdefMethod.args = [arg];
            //     // funcdefMethod.
            //     funcdefMethod.moduleOffset = scope.previous.start_offset + signature.start;
            //     funcdefMethod.moduleOffsetEnd = scope.previous.start_offset + signature.end;
            //     typedb.AddTypeToDatabase(scope.getNamespace(), )
